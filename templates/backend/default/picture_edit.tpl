...
{/literal}
 ...
 <div id="rightcontent">
    ...
    <div align="center">
    <div id="simpleForm">
    <form name="edit_picture" method="POST" enctype="multipart/form-data" action="picture.php?m=edit&vid={$vid}">
    <fieldset>
    <legend>Picture Information</legend>
        <label for="title">标题: </label><input type="hidden" name="vid" value="{$vid}"/>
        ...
        <textarea type="text" name="des" style="width:50%;height:150px;">{$picture.des}</textarea><br>
        
        <label for="category_id">类型: </label>
        <select name="category_id" id="category_id">
        <option value="0">-----</option>
        {section name=i loop=$channels}
        	<option value="{$channels[i].CHID}" {if $picture.category_id == $channels[i].CHID}selected{/if}>{$channels[i].name}</option>
        {/section}
        </select><br>
        
		<label for="keyword" style="margin-right:10px;">图片编辑区: </label>
        <textarea id="description" name="description" style="width:200px;height:400px;visibility:hidden;">{$picture.description}</textarea><br>
    </fieldset>
    <div style="text-align: center;">
        <input type="submit" name="edit_picture" value="Edit picture" class="button">
    </div>
    </form>
    </div>
    </div>
