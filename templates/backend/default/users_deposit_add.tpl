     <div id="rightcontent">
        ...
        <div align="center">
        <div id="simpleForm">
        <form name="add_deposit" method="POST" enctype="multipart/form-data" action="users.php?m=deposit_add&uid={$uid}">
        <fieldset>
        <legend>添加用户存款记录--{$username}({$user_range})</legend>
            <label for="money" style="width:35%;">本次存款金额: </label>
            <input type="text" name="money" class="medium"><br><input name="uid" type="hidden" value="{$uid}" /><input name="add_deposit" type="hidden" value="add_deposit"/>
            <label for="isget_sebi" style="width:35%;">是否玩游戏赠送色币: </label>
            <input type="checkbox" name="isget_sebi" value="1" style="box-sizing: border-box;position:relative;top:8px;margin-left:10px;"><br> 
            <label for="sebi" style="width:35%;">本次存款所得色币: </label>
            <input type="text" name="sebi" class="medium"/><br>
            
            <label for="get_sebi" style="width:35%;">玩游戏所得色币: </label>
            <input type="text" name="get_sebi" class="medium" /><br>
				
            <label for="dtime" style="width:35%;">存款时间: </label>
            <input type="text" name="dtime" id="dtime" class="medium"><br>
        </fieldset>
        <div style="text-align: center;">
            <input type="button" name="add_deposit" value="添加记录" onclick="check_submit();return false;" class="button">
        </div>
        </form>
        </div>
        </div>
...
<script type="text/javascript" src="/templates/backend/default/js/jquery-1.11.1.min.js"></script>
